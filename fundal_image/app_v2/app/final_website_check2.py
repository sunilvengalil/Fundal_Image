...
from utils import read_image, read_mask, save_results_zip, save_results_zip_no_gt, \
    save_results_od_bv, calculate_metrics_od_bv, confusion_matrix
...
if _radio == "Home":
    ...
    model_dir = "models"
    model_path = f"{model_dir}/model_1024.h5"
    od_model_channel = 0
    bv_model_channel = 1
    bv_model = None
    od_model = None
    ...
    fold_var = 0
    quality_check_model = load_model(save_dir + get_model_name(fold_var),
    ...
    if radio_upload == "Upload Image For Prediction":
        ...
        if img_data is not None and len(img_data) > 0:
            ...
            with st.form(key="quality_check"):
                ...
                with st.spinner('Performing Image Quality Analysis on uploaded Image...'):
                    ...
                    image_quality, quality_score = get_image_quality(img_data, img, quality_check_model)
            ...
            with st.form(key="bv_segmentation"):
                ...
                st.code(f"Quality Score: {quality_score:0.2f} [Score 0-1 - O is poor and 1 is best]"
                        , language="python")
                ...
                with st.spinner('Computing the Prediction Results...'):
                    ...
                    y_pred = segmentation_model.predict(np.expand_dims(x, axis=0))[0]
                    #new_y_pred = cv2.cvtColor(y_pred, cv2.COLOR_BGR2GRAY)
                    ...
                    if bv_checkbox:
                        # if bv_model is None:
                        #     print(f"Loading Blood Vessel Segmentation model from {model_path}")
                        #     bv_model = load_model(model_path, compile=False)
                        ...
                        if len(bv_data) == 0:
                            pred_name_bv = f"history/bv_prediction_{count}.jpg"
                            save_results_od_bv(y_pred_binary, pred_name_bv, mode="single", ori_y=None, channel=1)
                            ...
                            try:
                                ...
                                save_results_od_bv(y_pred_binary, pred_name_bv_combined, mode="combined",
                                                   ori_y=ori_y_bv, channel=1)
                        ...
                        if len(bv_data) > 0:
                            try:
                                ...
                                print("This are the following results we get ")

                                cf_Matrix = confusion_matrix(y_pred[:,:,1],y_bv,0.1)
                                # print("This are the following results we get ")
                                print(y_pred.shape)
                                print(y_bv.shape)
                                ...
                                print(np.max(y_pred))
                                print(cf_Matrix)
                                SCORE = calculate_metrics_od_bv(y_bv, y_pred_binary, count, 1)
                    ...
                    if od_checkbox:
                        ...
                        if len(od_data) == 0:
                            pred_name_od = f"history/od_prediction_{count}.jpg"
                            save_results_od_bv(y_pred_binary, pred_name_od, mode="single", ori_y=None, channel=0)
                            ...
                            try:
                                ...
                                save_results_od_bv(y_pred_binary, pred_name_od_combined, mode="combined",
                                                   ori_y=ori_y_od, channel=0)
                                ...
                                st.write("Waiting for upload of ground truth...")
                        # Fix this channel number is wrong
                        if len(od_data) > 0:
                            try:
                                ...
                                SCORE = calculate_metrics_od_bv(y_od, y_pred_binary, count, 0)
        ...
        model_dir = "models"
        model_path = f"{model_dir}/bv_model.h5"
        bv_model = load_model(model_path, compile=False)
        od_model = load_model("models/od_model.h5", compile=False)
            ...
            with st.spinner('Computing the Prediction Results...'):
                ...
                if bv_value == True:
                    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
                        ...
                        y_pred_binary = bv_model.predict(np.expand_dims(x, axis=0))[0]
                        y_pred_binary = y_pred_binary > 0.5
                        y_pred_binary = y_pred_binary.astype(np.int32)
                        y_pred_binary = np.squeeze(y_pred_binary, axis=-1)

                        save_image_path = f"results/{name}.png"

                        save_results_zip(ori_x, ori_y, y_pred_binary, save_image_path)

                        y = y.flatten()
                        y_pred_binary = y_pred_binary.flatten()
                        acc_value = accuracy_score(y, y_pred_binary)
                        f1_value = f1_score(y, y_pred_binary, labels=[0, 1], average="binary")
                        jac_value = jaccard_score(y, y_pred_binary, labels=[0, 1], average="binary")
                        recall_value = recall_score(y, y_pred_binary, labels=[0, 1], average="binary")
                        precision_value = precision_score(y, y_pred_binary, labels=[0, 1], average="binary")
                    ...
                    for x in tqdm(test_x, total=len(test_x)):
                        ...
                        save_results_zip_no_gt(ori_x, y_pred_binary, save_image_path)
        ...
        if od_zip_checkbox:
            ...
            dataset_path = "data"
            test_x, test_y = load_data(dataset_path, default=od_value)
            ...
            with st.spinner('Computing the Prediction Results...'):
                ...
                if od_value == True:
                    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
                        ...
                        y_pred_binary = od_model.predict(np.expand_dims(x, axis=0))[0]
                        y_pred_binary = y_pred_binary > 0.5
                        y_pred_binary = y_pred_binary.astype(np.int32)
                        y_pred_binary = np.squeeze(y_pred_binary, axis=-1)
    ...
    if st.button('Submit Feedback'):
        st.write("Thank You For submitting the feedback! Your feedback has been registered successfully!")
