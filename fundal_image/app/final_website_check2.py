...
from utils import read_image, read_mask, confusion_matrix, save_results, calculate_metrics, save_results_zip, save_results_zip_no_gt,calculate_metrics_od_bv
from utils import save_results_zip_no_gt_od_bv, save_results_zip_no_gt, save_results_od_bv, save_results_zip_od_bv
...
if _radio == "Home":
    ...
    fold_var = 0
    print("Print loading modle from ", save_dir + get_model_name(fold_var))
                    ...
                    if bv_checkbox:
                        ...
                        if len(bv_data) > 0:
                            try:
                                ...
                                print(np.max(y_pred))
                                print(cf_Matrix)
                                SCORE = calculate_metrics_od_bv(y_bv, y_pred_binary, threshold,  count, 1)
                    ...
                    if od_checkbox:
                        ...
                        if len(od_data) > 0:
                            try:
                                ...
                                # y_od = y_od.flatten()
                                # y_pred = y_pred.flatten()
                                print(y_od.shape, y_pred_binary.shape)
                                SCORE = calculate_metrics_od_bv(y_od, y_pred_binary, threshold, count, 0)
            ...
            with st.spinner('Computing the Prediction Results...'):
                ...
                if bv_value == True:
                    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
                        ...
                        save_results_zip_od_bv(ori_x, ori_y, y_pred, save_image_path, count, 1)
                    ...
                    for x in tqdm(test_x, total=len(test_x)):
                        ...
                        save_results_zip_no_gt_od_bv(ori_x, y_pred, save_image_path, channel=1)
            ...
            with st.spinner('Computing the Prediction Results...'):
                ...
                if od_value == True:
                    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
                        ...
                        # save_results_zip(ori_x, ori_y, y_pred, save_image_path)
                        save_results_zip_od_bv(ori_x, ori_y, y_pred, save_image_path, count, 0)
                    ...
                    for x in tqdm(test_x, total=len(test_x)):
                        ...
                        save_results_zip_no_gt_od_bv(ori_x, y_pred, save_image_path, channel=0)
